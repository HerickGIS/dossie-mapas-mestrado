...
if "last_draw" not in st.session_state: st.session_state["last_draw"] = None
...
# =====================================================================
# 3. MOTOR UNIVERSAL DE CORES E CARTOGRAFIA
# =====================================================================
...
def obter_coluna_real(gdf):
    ...
    colunas_validas = extrair_colunas_validas(gdf)
...
if modo_analise == "1. Visão Geral":
    ...
    with st.expander("📖 Metodologia da Pesquisa Resumida - A Álgebra de Mapas com a Ecodinâmica", expanded=False):
        st.markdown("A pesquisa deu através das análises, mediante a aquisição do banco de dados geográficos, onde foi possível através dos softwares do ArcGIS e QGIS, usufruindo de suas ferramentas e recursos, a elaboração dos Mapas de Localização, Hipsometria e Hidrografia, Declividade, Pluviometria, Geologia, Geomorfologia, Pedologia, Cobertura da Vegetação, Uso e Cobertura da Terra, Vulnerabilidade Natural e Vulnerabilidade Ambiental. 
        A partir de todos os dados, para o objetivo final da pesquisa, utilizou-se a modelagem de vulnerabilidade que integra os dados de todos os principais mapas com a dimensão do meio físico e a pressão antrópica por Álgebra de Mapas
        Para mais informações, consultar o documento da Dissertação no canto inferior esquerdo.")
    ...
    camadas_alvo = st.sidebar.multiselect(
        "Selecione os dados para visualizar:", 
        list(mapas_encontrados.keys()), 
        default=[bacia_key],
        help="Escolha múltiplas camadas para sobrepô-las no mapa cartográfico."
    )

    estilos_camadas = {}
    for nome_camada in camadas_alvo:
        with st.sidebar.expander(f"🎨 Estilo: {nome_camada}", expanded=False):
            opacidade = st.slider("Transparência", 0.0, 1.0, 0.7, key=f"op_{nome_camada}", help="Reduza para ver as camadas de baixo.")
            tipo_cor = st.radio("Cores:", ["Por Atributo", "Cor Única"], key=f"tc_{nome_camada}", help="Defina se o mapa será monocromático ou terá cores baseadas nas classes.")
